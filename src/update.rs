...
fn fetch_latest_release() -> Result<Release> {
  ...
  let assets = resp["assets"].as_array()
    .map(|arr: &Vec<serde_json::Value>| arr.iter().filter_map(|a| {
...
fn download_and_replace(asset_url: &str) -> Result<()> {
  ...
  let binary_name = if cfg!(target_os = "windows") { "dodo.exe" } else { "dodo" };
  let mut found = false;
...
pub fn check_update() -> Result<()> {
  ...
  println!("Update available: v{CURRENT_VERSION} → v{}", release.version);
  println!("Downloading...");

  let target = detect_target()?;
  if cfg!(target_os = "windows") {
    println!("Self-update is not supported on Windows.");
    println!("Download the latest release: https://github.com/{REPO}/releases/tag/{}", release.tag);
    return Ok(());
  }
