...
- `src/db.rs` — SQLite database (turso), migrations, all queries, session lifecycle
...
- **Recurrence notation**: `parse_recurrence()` validates patterns. `next_occurrence()` computes the next date from a pattern + reference date. Day-of-month clamps to last day (e.g., day31 in Feb → Feb 28). **Monthly drift caveat**: `*monthly` and `*Nm` patterns use `checked_add_months` which clamps to the last day of the target month. Once the anchor crosses February on a day 29-31, it permanently drifts toward day 28 (e.g. Jan 31 → Feb 28 → Mar 28, not Mar 31). For stable end-of-month recurrence, use `*day31` instead — it skips short months and lands precisely on day 31 of each eligible month.
- **Instance indicator**: Recurring instances show `↻` at the **beginning** of the title in task panes (styled with inverted green).
- **Database engine**: Uses `turso` crate (SQLite-compatible rewrite with native push/pull sync). `Database` struct stores `turso::Database` handle + `Connection` + `tokio::runtime::Runtime`. Main DB (`dodo.db`) is always `Builder::new_local()` — zero network latency. Sync opens the same `dodo.db` via `turso::sync::Builder::new_remote()` with push/pull (CDC-based, no separate replica file). All DB methods use an async bridge: each method wraps async turso calls in `self.rt.block_on()`. Row value extraction uses helper functions (`val_string`, `val_i64`, `val_bool`, `val_opt_string`, `val_opt_i64`, `val_opt_bool`) with turso's `Value` enum for nullable fields. Methods using transactions (`pause_timer`, `complete_task`, `complete_task_by_id`, `set_elapsed_seconds_by_id`, `update_template_fields`, `start_timer`) require `&mut self` because turso's `Connection::transaction()` takes `&mut self`.
...
- **Turso sync (local-first, non-blocking)**: Single-DB architecture using `turso` crate with CDC-based push/pull. `do_remote_sync(url, token)` opens `dodo.db` via `turso::sync::Builder::new_remote()`, calls `pull()` then `push()`, then runs `dedup_active_recurring_instances()`. Conflict resolution is "last push wins" with rollback-and-replay on pull (handled by turso engine). `sync_with_remote()` spawns this in a `std::thread`, returns `mpsc::Receiver<Result<()>>`. TUI stores receiver in `App.sync_receiver`, polls with `check_sync_result()` via `try_recv()` (non-blocking). CLI `dodo sync now` calls `do_remote_sync()` directly (blocking). No final sync on TUI quit — data is safe locally, syncs on next startup.
- **Sync tombstones**: `deleted_tasks` table tracks IDs of locally deleted tasks. `delete_task_by_id()` and `delete_template()` record tombstones before deleting. With turso CDC sync, DELETE statements propagate natively — tombstones are primarily for application-level dedup logic (`dedup_active_recurring_instances()`). `merge_remote_data()` is retained for backup restore and test use but is no longer part of the sync path.
...
- turso (SQLite-compatible) stored at `~/.local/share/dodo/dodo.db` (always local)
- No separate sync replica file — turso opens the same `dodo.db` with sync capabilities
