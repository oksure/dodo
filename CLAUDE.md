...
- `src/config.rs` — config file parsing (`~/.config/dodo/config.toml`) for sync, backup, and preferences settings (`WeekStart` enum, `PreferencesConfig` with `sound_enabled`, `timer_sound_interval`, `default_view`, `default_estimate`)
...
- `tests/config_test.rs` — 37 unit tests for config parsing, defaults, is_ready checks, serialization roundtrip, PreferencesConfig/WeekStart/sound/view/estimate
...
- `cargo test` runs all 189 tests
- `src/backup.rs` (inline) — 25 unit tests for format_size, format_age, parse_backup_timestamp
- `tests/config_test.rs` — 27 unit tests for config parsing, TOML deserialization, defaults, is_ready checks, serialize/deserialize roundtrip, PreferencesConfig/WeekStart
